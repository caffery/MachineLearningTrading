...
def get_portfolio_value(prices, allocs, start_val=1):
    ...
    """
    #First, get cumulative returns for each stock in portfolio
    returns = prices / prices.ix[0]
    
    #Get daily portfolio returns
    portfolio_returns = (returns * allocs).sum(axis=1)
...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    """
    #Get daily returns
    daily_ret = ( port_val - port_val.shift(1) ) / port_val.shift(1)
    daily_ret = daily_ret[1:] #drop first value, it is NaN
    
    cum_ret = float(port_val[-1])/port_val[0] - 1.0
    avg_daily_ret = daily_ret.mean()
    std_daily_ret =  daily_ret.std()
    sharpe_ratio = samples_per_year / np.sqrt(samples_per_year) * (avg_daily_ret - daily_rf) / std_daily_ret
...
def assess_portfolio(start_date, end_date, symbols, allocs, start_val=1):
    ...
    cum_ret, avg_daily_ret, std_daily_ret, sharpe_ratio = get_portfolio_stats(port_val, samples_per_year)
...
def test_run():
    ...
    #I do not know why but GOOG is not working, I use GOOGL instead
    symbol_allocations = OrderedDict([('GLD', 0.4), ('XOM', 0.1), ('GOOGL', 0.2), ('AAPL', 0.3) ])  # symbols and corresponding allocations
